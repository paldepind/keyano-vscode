...
function getSvgDataUri(
  ...
  return vscode.Uri.parse(
    `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} 13" height="13" width="${width}"><rect width="${width}" height="13" rx="2" ry="2" style="fill: ${backgroundColor};"></rect><text font-family="Fira Mono" font-size="11px" fill="${fontColor}" x="1" y="10">${code}</text></svg>`
  );
}
...
function createDataUriCaches(codes: string[]) {
  codes.forEach(
    (code) =>
      (darkDataUriCache[code] = getSvgDataUri(
        code,
  ...
  codes.forEach(
    (code) =>
      (lightDataUriCache[code] = getSvgDataUri(
        code,
        "rgba(0, 0, 0, 0.8)",
        "white"
      ))
...
function createDecorationOptions(
  from: Position,
  to: Position,
  text: string
...
export function goToTarget(givenKeys: string, targets: Target[]) {
  ...
  editor.selection = new Selection(
    document.positionAt(target.from),
    document.positionAt(target.to)
  );
}
